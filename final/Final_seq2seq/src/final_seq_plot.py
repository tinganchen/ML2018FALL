...
'''
model.save('savings/s2s_v2.h5') # 'savings/s2s.h5'
...
with open('savings/s2s_checkpoints.txt','w') as f:
    f.write(str(model_trained.history))

# model.save('s2s.h5') # 'savings/s2s.h5'
with open('savings/s2s_checkpoints.txt','r') as f:
...
plt.savefig('savings/Seq2seq_losses.png')
...
plt.ylabel('Accuracy')
plt.xticks(np.arange(0, 10, step = 1), np.arange(0, 10, step = 1) + 1)
plt.savefig('savings/Seq2seq_acc.png')
