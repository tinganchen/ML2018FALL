...
encoder = LSTM(latent_dim, return_sequences = True, dropout = 0.3, recurrent_dropout = 0.3)
encoder_outputs = encoder(encoder_inputs)


decoder_inputs = Reshape([10, 8*latent_dim])(encoder_outputs)

decoder_lstm = LSTM(2048, return_sequences = True, dropout = 0.3, recurrent_dropout = 0.3)
decoder_outputs = decoder_lstm(decoder_inputs)
outputs = Dense(num_words, activation = 'softmax')(decoder_outputs)
...
def data_loader(encoder_input, decoder_output, batch_size):
    ...
    while True:
        for step in range(steps):
            ...
            yield encoder_input_batch, decoder_output_batch
...
'''
# model.save('savings/s2s_no_cap_inputs.h5') # epoch = 10

model.save('savings/s2s_no_cap_inputs_epoch_3.h5') # epoch = 3

model_trained = model.fit_generator(generator = data_loader(encoder_input_train, decoder_target_train, batch_size),                  
                                    validation_data = (np.array(encoder_input_valid), np.array(decoder_target_valid)), 
                                    steps_per_epoch = int(len(encoder_input_train) / batch_size),
                                    epochs = epochs) 
'''
with open('savings/s2s_no_cap_inputs_checkpoints.txt','w') as f:
    f.write(str(model_trained.history))

with open('savings/s2s_no_cap_inputs_checkpoints.txt','r') as f:
    training_eval = f.read()
'''
with open('savings/s2s_no_cap_inputs_checkpoints_epoch_3.txt','w') as f:
    f.write(str(model_trained.history))

with open('savings/s2s_no_cap_inputs_checkpoints_epoch_3.txt','r') as f:
    training_eval = f.read()
    
train_eval = yaml.load(training_eval)

'''Epoch = 10
# Seq2seq - Loss
plt.plot(train_eval['loss'], label = 'Training Loss')
plt.plot(train_eval['val_loss'], label = 'Validation Loss')
plt.legend()
plt.title('Seq2seq without Captions Inputs - Loss')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.xticks(np.arange(0, 10, step = 1), np.arange(0, 10, step = 1) + 1)
#plt.yticks(np.arange(5, 6.5, step = 0.5) / 10, np.arange(5, 6.5, step = 0.5) / 10)
plt.savefig('savings/Seq2seq_no_cap_losses.png')

# Seq2seq - Accuracy
plt.plot(train_eval['acc'], label = 'Training Accuracy')
plt.plot(train_eval['val_acc'], label = 'Validation Accuracy')
plt.legend()
plt.title('Seq2seq without Captions Inputs - Accuracy')
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
plt.xticks(np.arange(0, 10, step = 1), np.arange(0, 10, step = 1) + 1)
plt.savefig('savings/Seq2seq_no_cap_acc.png')
'''
...
plt.plot(train_eval['val_loss'], label = 'Validation Loss')
plt.legend()
plt.title('Seq2seq without Captions as Inputs - Loss')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.xticks(np.arange(0, 10, step = 1), np.arange(0, 10, step = 1) + 1)
#plt.yticks(np.arange(5, 6.5, step = 0.5) / 10, np.arange(5, 6.5, step = 0.5) / 10)
plt.savefig('savings/Seq2seq_no_cap_losses_epoch_3.png')

# Seq2seq - Accuracy
plt.plot(train_eval['acc'], label = 'Training Accuracy')
plt.plot(train_eval['val_acc'], label = 'Validation Accuracy')
plt.legend()
plt.title('Seq2seq without Captions as Inputs - Accuracy')
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
plt.xticks(np.arange(0, 10, step = 1), np.arange(0, 10, step = 1) + 1)
plt.savefig('savings/Seq2seq_no_cap_acc_epoch_3.png')
