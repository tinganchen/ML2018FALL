...
def get_optimizer():
    ...
    optimizer = tf.train.GradientDescentOptimizer(learning_rate=lr) # optimizer
    return optimizer
...
def before_epoch(train_history, validation_history):
    ...
    return {"transform": transform, "batchsize": 20} # batch_size
...
def before_batch(train_history, validation_history):
    ...
    """
    global lr
    return {'feed_dict': {lr: 0.07}, 'batch_norm': 0.9, 'drop_out': 0.5}
...
def save_model_as(train_history, validation_history):
    ...
    for i in range(n_epoch):
        if n_epoch == i+1:
            return 'save_mobile_{}'.format(i+1)
    """